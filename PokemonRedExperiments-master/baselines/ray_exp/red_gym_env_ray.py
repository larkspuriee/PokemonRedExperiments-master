...
class RedGymEnv(gym.Env):
    ...
    def update_frame_knn_index_and_get_reward(self, frame_vec):
        novelty_reward = 0
        ...
        if self.knn_index.get_current_count() == 0:
            # Add the first frame to the index
            self.knn_index.add_items(frame_vec, np.array([self.knn_index.get_current_count()]))
            novelty_reward = 1.0  # High reward for the first state
        else:
            # Query nearest neighbor distance and compute reward
            labels, distances = self.knn_index.knn_query(frame_vec, k=1)
            if distances[0][0] > self.similar_frame_dist:
                # Novel state, add to index and give a reward
                self.knn_index.add_items(frame_vec, np.array([self.knn_index.get_current_count()]))
                novelty_reward = 1.0 - min(distances[0][0] / self.similar_frame_dist, 1.0)
            else:
                # Less novel state, smaller reward
                novelty_reward = 0.1 * (1.0 - distances[0][0] / self.similar_frame_dist)
